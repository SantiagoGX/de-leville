(() => {
          ...
          cartButtons.forEach(function(btn) {
            ...
            if (itemCount > 0) {
              if (!countEl) {
                ...
                countEl.style.cssText = 'position:absolute;top:-5px;right:-5px;background:#000000;color:white;border-radius:50%;width:18px;height:18px;font-size:11px;display:flex;align-items:center;justify-content:center;font-weight:500;';
  ...
  api.register('snc-wishlist', (section) => {
    ...
    async function renderWishlist() {
      ...
      grid.innerHTML = '';
      grid.innerHTML = '<div class="snc-plp--loader"><div class="snc-plp--spinner"></div><p>Cargando tus Favorites...</p></div>';
      ...
      if (favorites.length > 0) {
        try {
          ...
          items.forEach(product => {
            ...
            if (cardStyle === 'below') {
              if (sizeIndex >= 0 && colorIndex >= 0) {
                ...
                qaOverlayHtml = `
                  <div class="snc-plp--quick-add-overlay">
                    <button type="button" class="snc-plp--qa-btn-full" data-add-product-handle="${escapeHtml(product.handle)}">Added to your bag</button>
                  </div>
                `;
              }
            }
          ...
          grid.innerHTML = '<p>Error al cargar Favorites.</p>';
        }
      }
    }
  ...
  api.register('snc-pdp-hero', (heroRoot) => {
    ...
    if (addToCartButton) {
      // De Leville: ring-size disclosure — trigger + pill grid + unit toggle → hidden input
      // All handlers use event delegation on heroRoot so they survive [data-pdp-size-zone]
      // innerHTML swaps performed by PDPAjax.zone() on variant navigation.

      function getSizeEls() {
        return {
          selectorWrap:  heroRoot.querySelector('[data-size-selector]'),
          sizeSelect:    heroRoot.querySelector('[data-ring-size]'),
          trigger:       heroRoot.querySelector('[data-size-trigger]'),
          triggerLabel:  heroRoot.querySelector('[data-size-trigger-label]'),
          panel:         heroRoot.querySelector('[data-size-panel]'),
        };
      }

      function openPanel() {
        const els = getSizeEls();
        if (!els.panel || !els.trigger) return;
      ...
      // Apply placeholder class to initial trigger
      const initTrigger = heroRoot.querySelector('[data-size-trigger]');
      if (initTrigger) initTrigger.classList.add('is-placeholder');

      // Single delegated listener on heroRoot — handles trigger, pills, and unit buttons
      on(heroRoot, 'click', function(e) {
        if (e.target.closest('[data-size-trigger]')) {
          e.stopPropagation();
          const els = getSizeEls();
          if (els.trigger && els.trigger.getAttribute('aria-expanded') === 'true') closePanel();
          else openPanel();
          return;
        }

        const pill = e.target.closest('[data-size-pills] .pdp-hero--size-pill');
        if (pill) {
          const els = getSizeEls();
          heroRoot.querySelectorAll('[data-size-pills] .pdp-hero--size-pill').forEach(function(p) {
            p.classList.remove('is-selected');
            p.setAttribute('aria-checked', 'false');
          });
          pill.classList.add('is-selected');
          pill.setAttribute('aria-checked', 'true');
          const us = pill.getAttribute('data-us');
          const eu = pill.getAttribute('data-eu');
          const val = 'US ' + us + ' / EU ' + eu;
          if (els.sizeSelect) els.sizeSelect.value = val;
          if (els.triggerLabel) els.triggerLabel.textContent = val;
          if (els.trigger) els.trigger.classList.remove('is-placeholder');
          if (els.selectorWrap) els.selectorWrap.classList.remove('has-error');
          closePanel();
          return;
        }

        const unitBtn = e.target.closest('[data-unit-btn]');
        if (unitBtn) {
          e.stopPropagation();
          const els = getSizeEls();
          const unit = unitBtn.getAttribute('data-unit-btn');
          if (els.selectorWrap) els.selectorWrap.setAttribute('data-unit', unit);
          heroRoot.querySelectorAll('[data-unit-btn]').forEach(function(b) {
            const active = b.getAttribute('data-unit-btn') === unit;
      ...
      document.addEventListener('click', function(e) {
        const els = getSizeEls();
        if (!els.selectorWrap) return;
        if (!els.selectorWrap.contains(e.target)) closePanel();
      });
      ...
      on(addToCartButton, 'click', function() {
        ...
        // Use live queries — [data-pdp-size-zone] may have been swapped by PDPAjax
        const atcEls = getSizeEls();

        // De Leville rule: if no size is selected, don't add to bag.
        // Instead scroll the size selector into view, highlight it with the
        // error style, and focus the hidden select so keyboard users can
        // pick a size immediately. Primary + sticky-bar clicks both run this
        // path because the sticky bar forwards clicks to this button.
        if (atcEls.sizeSelect && !atcEls.sizeSelect.value) {
          if (atcEls.selectorWrap) {
            atcEls.selectorWrap.classList.add('has-error');
            atcEls.selectorWrap.classList.add('is-pulsing');
            try {
              atcEls.selectorWrap.scrollIntoView({ behavior: 'smooth', block: 'center' });
            } catch (e) {
              atcEls.selectorWrap.scrollIntoView();
            }
            // Drop the pulse class after the animation so repeat clicks re-trigger it.
            setTimeout(function () {
              atcEls.selectorWrap.classList.remove('is-pulsing');
            }, 1200);
          }
          // Open the size dropdown so the user doesn't need an extra tap.
          if (atcEls.trigger && atcEls.trigger.getAttribute('aria-expanded') !== 'true') {
            setTimeout(function () { openPanel(); }, 420);
          }
          if (atcEls.sizeSelect) atcEls.sizeSelect.focus();
          return;
        }
        ...
        const payload = { items: [{ id: selectedVariantId, quantity: 1 }] };
        if (atcEls.sizeSelect && atcEls.sizeSelect.value) {
          payload.items[0].properties = { 'Size': atcEls.sizeSelect.value };
        }
    ...
    if (deliveryElement) {
      // De Leville: made-to-order. No business-day countdown, no localized dates.
      ...
      deliveryElement.textContent = 'Made to order. Delivered in 4\u20136 weeks.';
    }
    ...
    on(document, 'snc:show-toast', (event) => {
      showToast(event.detail.message || 'Added to your bag');
    });
  ...
  api.register('snc-scroll-gallery', (section) => {
    const { on, cleanup } = makeTracker();
    const scrollAnimationEnabled = section.dataset.scrollAnimation === 'true';
    
    if (scrollAnimationEnabled) {
      const animatedElements = section.querySelectorAll('.snc-scroll-gallery--animate, .snc-scroll-gallery--image');
